...
async def generate_cryptocloud_invoice(amount, user_id, coin_amount):
    ...
    # Include the user_id and coin amount in the order_id
    # This will be used to identify the user when processing the webhook
    order_id = f"user_{user_id}_{coin_amount}"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("status") == "success":
                logger.info(f"Created CryptoCloud invoice for user {user_id}, amount: {amount}, coins: {coin_amount}")
                # Return the payment link generated by CryptoCloud
                return result["result"]["link"]
        
        logging.error(f"CryptoCloud API error: {response.status_code} - {response.text}")
        return None
    except Exception as e:
        logging.error(f"Error generating CryptoCloud invoice: {e}")
        return None
...
async def process_payment(callback: CallbackQuery, state: FSMContext):
    ...
    # Generate payment link based on the selected method
    if payment_method == "cryptocloud":
        payment_link = await generate_cryptocloud_invoice(price_usd, callback.from_user.id)
    elif payment_method in ["wata", "payeer", "b2pay"]:
        # Placeholder for other payment methods
        # In a real implementation, you would integrate with these payment providers
        payment_link = f"https://example.com/placeholder-payment/{payment_method}/{price_usd}"
    ...
    if payment_link:
        ...
        success_text = (
            ...
            f"Price: {hcode(f'${price_usd:.2f}')}\n"
            f"Payment method: {hbold(payment_data.get('payment_method_name'))}\n\n"
            f"Click the button below to proceed to the payment page. After successful payment, the coins will be credited to your balance."
        )
